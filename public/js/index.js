            ...
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    if (!JSON.parse(xhr.responseText)) {
                        window.location.href = 'public/images/site/error-404.html'
                    } else {
...
document.querySelectorAll(".ajaxImg").forEach((elt) => {
    elt.addEventListener("click", () => {
        switch (elt.id) {
            case 'photo1':
                document.querySelector('#img-principal>img').setAttribute('src', pathPhoto1);
                break;
            case 'photo2':
                document.querySelector('#img-principal>img').setAttribute('src', pathPhoto2);
                break;
            case 'photo3':
                document.querySelector('#img-principal>img').setAttribute('src', pathPhoto3);
                break;
            case 'photo4':
                document.querySelector('#img-principal>img').setAttribute('src', pathPhoto4);
                break;
            case 'photo5':
                document.querySelector('#img-principal>img').setAttribute('src', pathPhoto5);
                break;
        }
