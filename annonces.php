...
//    if (!empty($_FILES['photo_4']['name']) && !in_array($_FILES['photo_4']['type'], $ext_auto, true) && ($_FILES['photo_4']['size'] > 100000)) {
//        $errors++;
//        add_flash('Pour la photo 4 Format autorisé: JPEG ou PNG dont la taille ne dépasse pas les 100kb', 'danger');
//    }
//    if (!empty($_FILES['photo_5']['name']) && !in_array($_FILES['photo_5']['type'], $ext_auto, true) && ($_FILES['photo_5']['size'] > 100000)) {
//        $errors++;
//        add_flash('Pour la photo 5 Format autorisé: JPEG ou PNG dont la taille ne dépasse pas les 100kb', 'danger');
//    }
        ...
        if (isset($_GET['action']) && ($_GET['action'] == 'edit') && !empty($editAnnounce)) {
            ...
            $announces->updateAnnounce(
                [
                    ...
                    'id' => $editAnnounce['id_annonce']

                ],
                $editAnnounce['id_annonce'],
            );
...
?>
    ...
    <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
        <li class="nav-item me-1" role="presentation">
            <button class="announces nav-link <?= (!empty($_GET['action']) || !empty($_POST)) ? '' : 'active show' ?>"
                    id="pills-announces-tab" data-bs-toggle="pill"
                    data-bs-target="#pills-announces" type="button" role="tab" aria-controls="pills-announces"
                    aria-selected="true"><i class="fas fa-list me-1"></i>
                Vos annonces
            </button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="announces nav-link <?= (!empty($_GET['action']) || !empty($_POST))  ? 'active show' : '' ?>"
                    id="pills-add-announce-tab" data-bs-toggle="pill"
                <?php echo (isset($_GET['action']) && $_GET['action'] === 'edit') ? 'style="background-color: orange !important; color=white !important"' : "" ?>
                    data-bs-target="#pills-add-announce" type="button" role="tab" aria-controls="pills-add-announce"
                    aria-selected="false"><i
                        class="fas fa-plus-square me-1"></i> <?php echo (isset($_GET['action']) && $_GET['action'] === 'edit') ? 'Mettre à jour ' : "Créer votre annonce" ?>
            </button>
        </li>
    </ul>
    <div class="tab-content" id="pills-tabContent">
        ...
        # TAB ANNOUNCES
        -->
        <div class="tab-pane fade <?= (!empty($_GET['action']) || !empty($_POST)) ? '' : 'active show' ?>" id="pills-announces"
             role="tabpanel"
            ...
            <table class="table d-flex flex-column">
                ...
                <?php foreach ($announces->findAllByUserID($_SESSION['user']['id_membre']) as $announce): ?>
                    <tr class="card cardtab">
                        <td>
                            ...
                            <div class="d-flex justify-content-center justify-content-md-between flex-wrap">
                                <img class="img-owncategory"
                                     src="<?= 'public/images/annonces/' . $announce['id_membre'] . '/' . $announce['id_annonce'] . '-' . '1' . '-' . $announce['photo'] ?>"
                                     alt="<?= $announce['categorie_titre'] ?>">
                                <div class="d-flex flex-md-column justify-content-center align-items-stretch">
                                    <a id="edit-annonce" href="?action=edit&id=<?php echo $announce['id_annonce'] ?>"
                                       class="btn btn-warning m-1"><i class="fas fa-edit text-light"></i></a>
                                    <button href="#deleteAnnounce<?= $announce['id_annonce'] ?>" data-bs-toggle="modal"
                                            role="button" class="btn btn-danger m-1 delete-announce"><i
                    ...
                    <div class="modal fade" id="deleteAnnounce<?= $announce['id_annonce'] ?>" aria-hidden="true"
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-footer ">
                                    <button type="button" class="btn btn-danger me-1"
                                            onclick="deleteAnnounce(<?= $announce['id_annonce'] ?>)">supprimer
                                    </button>
        ...
        <div class="tab-pane fade mb-5 <?= (!empty($_GET['action']) || !empty($_POST)) ? 'active show' : '' ?>" id="pills-add-announce"
             role="tabpanel"
             aria-labelledby="pills-add-announce">
                ...
                <div class="row">
                    <div class="col-xl-6 col-12">
                        <div class="mb-3 group-value">
                            ...
                            <input name="name-announce-titre" type="text" class="form-control" id="announce-titre"
                                   placeholder="Ex: Maison de 120m² avec piscine proche des commerces"
                                   value="<?= $_POST['name-announce-titre'] ?? $editAnnounce['titre'] ?? '' ?>">
                        ...
                        <div class="mb-3 group-value">
                            ...
                            <textarea name="name-announce-description_courte" class="form-control"
                                      id="announce-description_courte" rows="2"
                                      placeholder="Faites une description précise et courte de votre annonce"><?= $_POST['name-announce-description_courte'] ?? $editAnnounce['description_courte'] ?? '' ?></textarea>
                            <div class="info text-danger announce-description_courte"
                                 data-error="errorDescription_courte"></div>
                        </div>
                        <div class="mb-3 group-value">
                            <label for="announce-description_longue" class="form-label">Description longue</label>
                            <textarea name="name-announce-description_longue" class="form-control"
                                      id="announce-description_longue" rows="4"
                                      placeholder="Description détaillée de votre annonce"><?= $_POST['name-announce-description_longue'] ?? $editAnnounce['description_longue'] ?? '' ?></textarea>
                            <div class="info text-danger announce-description_longue"
                                 data-error="errorDescription_longue"></div>
                        </div>
                        <div class="mb-3 group-value">
                            <label for="announce-prix" class="form-label">Prix</label>
                            <div class="input-group">
                                <input name="name-announce-prix" type="number" class="form-control" id="announce-prix"
                                       value="<?= $_POST['name-announce-prix'] ?? $editAnnounce['prix'] ?? '' ?>">
                        ...
                        <div class="mb-3 group-value">
                            ...
                            <select name="announce-id_categorie" class="form-select"
                    ...
                    <div class="col-xl-6 col-12">
                        <div class="row mb-3 justify-content-between py-2 border">
                            ...
                            <div class="col-12 col-sm-5 photo-announce group-value required-img" data-img="img-1">
                                ...
                                <label for="photo_1" class="bg-white photo-display-principal p-0 m-0"
                                    <?php if (!empty($editAnnounce['photo1']) && ($editAnnounce['photo1'] !== "")): ?>
                                        style='background: url(" <?= "public/images/annonces/" . $editAnnounce['id_membre'] . "/" . $editAnnounce['id_annonce'] . "-" . "1" . "-" . $editAnnounce['photo1'] ?> "); background-size: cover;'
                                    <?php elseif (!empty($_POST['data-img-1'])): ?>
                                       style='background: url(" <?= $_POST['data-img-1'] ?> "); background-size: cover;'
                                    <?php endif; ?>
                                    >
                                    <div class="h-100 d-flex align-items-center justify-content-center logo"
                                         data-img-logo="img-1">
                                        <i class="fas fa-camera fa-2x text-secondary"></i>
                                    </div>
                                </label>
                                <input name="photo_1" id="photo_1" class="form-control " type="file"
                                       hidden
                                       accept="image/jpeg,image/png">
                                <input type="hidden" name="nom-img-1" id="nom_principal" class="input-img-name"
                                       value="<?= $_POST['nom-img-1'] ?? $editAnnounce['photo1'] ?? '' ?>">
                                <input type="hidden" name="data-img-1" class="input-img"
                                       value="<?= $_POST['data-img-1'] ?? '' ?>">
                                <div class="info text-danger announce-image" data-error="errorPhoto"></div>
                            </div>
                        </div>
                        <div class="row mb-3 justify-content-around">
                            <div class="col-12 col-sm-6 col-md-3 justify-content-center photo-announce"
                                 data-img="img-2">
                                <div type="file" class="text-white text-center bg-secondary">photo 2</div>
                                <label for="photo_2"
                                       class="p-0 bg-white photo-display m-0 d-flex align-items-center justify-content-center"
                                    <?php if (!empty($editAnnounce['photo2']) && ($editAnnounce['photo2'] !== "")): ?>
                                        style='background: url(" <?= "public/images/annonces/" . $editAnnounce['id_membre'] . "/" . $editAnnounce['id_annonce'] . "-" . "2" . "-" . $editAnnounce['photo2'] ?> "); background-size: cover;'
                                    <?php elseif (!empty($_POST['data-img-2'])): ?>
                                       style='background: url(" <?= $_POST['data-img-2'] ?> "); background-size: cover;'
                                    <?php endif; ?>
                                    >
                                    <div class="h-100 d-flex align-items-center justify-content-center logo"
                                         data-img-logo="img-2">
                                ...
                                <input name="photo_2" id="photo_2" class="form-control" type="file" hidden
                                       accept="image/jpeg,image/png">
                                <input type="hidden" name="nom-img-2" id="nom-img-2" class="input-img-name"
                                       value="<?= $_POST['nom-img-2'] ?? $editAnnounce['photo2'] ?? '' ?>">
                                <input type="hidden" name="data-img-2" class="input-img"
                                       value="<?= $_POST['data-img-2'] ?? '' ?>">
                            ...
                            <div class="col-12 col-sm-6 col-md-3 justify-content-center photo-announce"
                                ...
                                <label for="photo_3"
                                       class="p-0 bg-white photo-display m-0 d-flex align-items-center justify-content-center"
                                    <?php if (!empty($editAnnounce['photo3']) && ($editAnnounce['photo3'] !== "")): ?>
                                        style='background: url(" <?= "public/images/annonces/" . $editAnnounce['id_membre'] . "/" . $editAnnounce['id_annonce'] . "-" . "3" . "-" . $editAnnounce['photo3'] ?> "); background-size: cover;'
                                    <?php elseif (!empty($_POST['data-img-3'])): ?>
                                       style='background: url(" <?= $_POST['data-img-3'] ?> "); background-size: cover;'
                                    <?php endif; ?>
                                    >
                                    <div class="h-100 d-flex align-items-center justify-content-center logo"
                                         data-img-logo="img-2">
                                ...
                                <input name="photo_3" id="photo_3" class="form-control" type="file"
                                       accept="image/jpeg,image/png" hidden>
                                <input type="hidden" name="nom-img-3" id="nom-img-3" class="input-img-name"
                                       value="<?= $_POST['nom-img-3'] ?? $editAnnounce['photo3'] ?? '' ?>">
                                <input type="hidden" name="data-img-3" class="input-img"
                                       value="<?= $_POST['data-img-3'] ?? '' ?>">
                            </div>
<!--BRIDAGE AU NIVEAU DU SERVEUR MUTUALISE 3 photo seulement en meme temps-->
                        ...
                        <div class="mb-2 group-value">
                            ...
                            <input name="announce-address" type="search" class="form-control" id="form-address"
                                   placeholder="Votre adresse complète ?"
                                   value="<?= $_POST['adresse'] ?? $editAnnounce['adresse'] ?? '' ?>">
                        ...
                        <div class="mb-3 group-value">
                            ...
                            <input name="announce-city" type="hidden" id="form-city" class="input-info"
                                   placeholder="Votre ville"
                                   value="<?= $_POST['ville'] ?? $editAnnounce['ville'] ?? '' ?>">
                        ...
                        <div class="mb-3 group-value">
                            ...
                            <input name="announce-pays" type="hidden" class="input-info" id="form-country"
                                   placeholder="Votre pays"
                                   value="<?= $_POST['pays'] ?? $editAnnounce['pays'] ?? '' ?>">
                        ...
                        <div class="mb-3 group-value">
                            ...
                            <input name="announce-cp" type="hidden" class="input-info" id="form-zip"
                                   value="<?= $_POST['cp'] ?? $editAnnounce['cp'] ?? '' ?>">


                            <input name="announce-latlng_lat" type="hidden" class="input-info" id="form-latlngLat"
                                   value="<?= $_POST['latlng_lat'] ?? $editAnnounce['latlng_lat'] ?? '' ?>">


                            <input name="announce-latlng_log" type="hidden" class="input-info" id="form-latlngLog"
                                   value="<?= $_POST['latlng_log'] ?? $editAnnounce['latlng_log'] ?? '' ?>">
